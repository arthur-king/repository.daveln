...
def resolve_url(url):
  ...
  if 'chiasenhac' in url:		
    try:
      mediaUrl=re.compile("\"hd-2\".+?\"([^\"]+)\"").findall(link)[0].replace('%3A',':').replace('%2F','/').replace('%2520','%20')
    except:
      mediaUrl=re.compile("\"file\".*?\"([^\"]*)\"").findall(link)[-1].replace('%3A',':').replace('%2F','/').replace('%2520','%20')
  if 'nhacso' in url:		
    mediaUrl=re.compile("src=\"([^\"]+)\" data-setup").findall(link)[0]		
