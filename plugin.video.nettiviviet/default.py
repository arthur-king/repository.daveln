...
import urllib,urllib2,re,os,sys,gzip,string
...
addon      = xbmcaddon.Addon('plugin.video.nettiviviet')
profile    = xbmc.translatePath(addon.getAddonInfo('profile'))
mysettings = xbmcaddon.Addon(id='plugin.video.nettiviviet')
...
tokenfile  = xbmc.translatePath(os.path.join(home, 'token.json'))
'''
status = urllib.urlopen('URL GOES HERE').getcode()
if status == 200:
	urllib.urlretrieve ('URL - SAME AS ABOVE - GOES HERE', tokenfile)
else:
	pass
'''
tokencode = open(tokenfile,"r").readlines()[0]
...
def tvlinks(url,name):
		...
		for url in match.findall(link):
				addLink(name,('rtmpe' + url + ' swfUrl=http://tv24.vn/getflash.ashx pageUrl=http://tv24.vn/ ' + tokencode),logos + 'tv24vn.png')
